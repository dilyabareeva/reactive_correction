...
import wandb
from src.utils.metrics import get_auc, get_auc_label
...
def train_model(
    ...
    for epoch in range(1, num_epochs + 1):
        metrics_epoch = run_one_epoch(
            model, dl_train, criterion, optimizer, device, update_params=True
        )
        ...
        if epoch % eval_every == 0:
            for val_name, dl_val in dl_val_dict.items():
                ...
                metrics_val = run_one_epoch(
                    model, dl_val, criterion, optimizer, device, update_params=False
                )
                metrics_val = {
                    f"{val_name}_{key}": val for key, val in metrics_val.items()
                }
                metrics_epoch = {**metrics_epoch, **metrics_val}

            if metrics_epoch["val_loss"] < best_loss:
                early_stop_count = 0
                best_loss = metrics_epoch["val_loss"]
            else:
                early_stop_count += 1
        ...
        if epoch % store_every == 0:
            store_model(
                model,
                f"{model_savedir}/checkpoint_{model_name}_{dataset_name}_{epoch}.pth",
            )
    ...
    store_model(
        model, f"{model_savedir}/checkpoint_{model_name}_{dataset_name}_last.pth"
    )
...
def run_one_epoch(
    ...
    results_auc = {
        f"AUC_{dl.dataset.class_names[class_id]}": get_auc_label(
