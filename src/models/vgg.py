...
def get_vgg(
    ...
    if (
        (not pretrained) & (ckpt_path is not None)
    ):  # TODO: introduce separate variables for loading imagenet and fine-tuning imagenet weights
        ...
        checkpoint = {
            k.replace("classifier.last", "classifier.6"): v
